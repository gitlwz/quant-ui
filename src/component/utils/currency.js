...
currency.prototype = {
  ...
  format: function format(useSymbol) {
    var _settings2 = this.s,
        formatWithSymbol = _settings2.formatWithSymbol,
        symbol = _settings2.symbol,
        separator = _settings2.separator,
        decimal = _settings2.decimal,
        groups = _settings2.groups;

    // set symbol formatting

    typeof useSymbol === 'undefined' && (useSymbol = formatWithSymbol);

    var values = ((useSymbol ? symbol : '') + this).split('.'),
        dollars = values[0],
        cents = values[1];
